...
def return_land(land_manager, returned_client_name, kitta_number, return_date, rent_start):
    for index, land in enumerate(land_manager.ltta and_data):
        if land['kitta_number'] == kitta_number and land['status'] == 'Not Available':
            rent_start = datetime.datetime.strptime(land[rent_start], '%Y-%m-%d %H:%M:%S')
            rent_duration = (return_date - rent_start).days // 30
            fine = max(0, (rent_duration - land['area']) * land['price'])  
            ...
            with open(invoice_file, 'w') as file:
                ...
                file.write(f"| {'KiNumber'.ljust(20)} | {str(kitta_number).ljust(22)} |\n")
